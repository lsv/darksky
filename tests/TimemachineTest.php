...
class TimemachineTest extends TestCase
{
     ...
     * @test
     */
    public function can_not_make_request_when_excluding_not_existing_block(
    ): void {
        $this->expectException(InvalidOptionsException::class);
        $this->expectExceptionMessageRegExp('/exclude/');

        $this->timemachine->excludeBlocks(['not_existing']);
        $this->timemachine->call(42.3601, -71.0589, new DateTime());
    }
    ...
    public function can_not_make_request_with_not_existing_language(): void
    {
        ...
        $this->timemachine->language('not_existing');
        $this->timemachine->call(42.3601, -71.0589, new DateTime());
    }
    ...
    public function can_not_make_request_with_not_existing_unit(): void
    {
        ...
        $this->timemachine->units('not_existing');
        $this->timemachine->call(42.3601, -71.0589, new DateTime());
    }
    ...
    {
        $this->timemachine->excludeBlocks(['alerts']);
        $this->timemachine->language('da');
        $this->timemachine->units('uk2');
        $this->timemachine->call(42.3601, -71.0589, new DateTime());
        $response = $this->timemachine->getResponse();
        $this->assertStringEqualsFile(__DIR__.'/stubs/timemachine.json',
            $response->getContent());
    }
     ...
     * @test
     */
    public function can_serialize_timemachine_response(): void
    {
        $response = $this->timemachine->call(42.3601, -71.0589, new DateTime());
        ...
        $this->assertInstanceOf(DateTime::class, $datapoint->time);
        $this->assertSame('Rain starting in the afternoon, continuing until evening.',
            $datapoint->summary);
        ...
        $this->assertStringStartsWith('...FLOOD WATCH REMAINS',
    ...
    {
        $response = new MockResponse(file_get_contents(__DIR__.'/stubs/timemachine.json'));
        $client = new MockHttpClient($response);
        $this->timemachine = new Timemachine('testing', $client);
    }
}
